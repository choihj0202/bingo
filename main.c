...
int main(int argc, char *argv[]) {
	...
	int count_user = 0;	// user의 빙고 수 계산에 대한 변수 선언  
	int count_com = 0;	// computer의 빙고 수 계산에 대한 변수 선언  
	...
	do 
	{
		...
		turn++;	// 턴 수 1 증가  
		count_user = count_bingo(user_bingo);	// user 빙고의 빙고 수 계산 
		count_com = count_bingo(com_bingo);	// computer 빙고의 빙고 수 계산  
		printf(" ## USER : %d 빙고 완성!\n\n ", count_user);	// user의 빙고 수 출력  
		printf(" ## COMPUTER : %d 빙고 완성!\n", count_com);	// computer의 빙고 수 출력  
