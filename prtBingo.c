...
// 초기 빙고 테이블을 만드는 함수
void initiate_bingo(int user[N][N], int com[N][N], int save_user[N*N], int save_com[N*N])	// save에 빙고 테이블의 각 숫자를 저장 		  
{  
	int i, j, k;
	...
	for (i=0; i<N; i++)
	{
		for (j=0; j<N; j++)
		{
			...
			else
			{
				...
				for (k=0; k<max; k++)
				{
					save_user[k] = user[i][j];	// save에 중복 여부 검사가 끝난 숫자들을 저장  
				}
			}
	...
	for (i=0; i<N; i++)
	{
		for (j=0; j<N; j++)
		{
			...
			else
			{
				...
				for (k=0; k<max; k++)
				{
					save_com[k] = com[i][j];	// save에 중복 여부 검사가 끝난 숫자들을 저장  
				}
			}
		}
	}
...
// 중복 숫자 출력을 막는 함수
int repeat(int num[N][N], int temp)	// temp는 배열의 정렬에 임시로 사용되는 변수  
{
	int i, j;
	...
	for (i=0; i<N; i++)
	{
		for (j=0; j<N; j++)
		{	
			if (num[i][j] == temp)	// 중복되는 숫자가 있으면 1을 반환하여 위의 if 문에 활용하기 용이하도록 함   
				return 1;
		}
	}
	return 0;
